...
- **Node 8 LTS:** `git-author-check` has a minimum Node version
...
``` bash
git clone https://github.com/EndemolShineGroup/git-author-check.git
cd git-author-check
yarn
yarn test
```
...
We want to ensure that `git-author-check` is secure for everyone. If
